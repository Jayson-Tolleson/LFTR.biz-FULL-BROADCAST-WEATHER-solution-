...
function locationsBbox(locations) {
  ...
  const lons = locations.map((l) => Number(l.lon)).filter(Number.isFinite);
  if (!lats.length || !lons.length) return null;
  const minLat = Math.max(-89.5, Math.min(...lats) - 0.75);
  const maxLat = Math.min(89.5, Math.max(...lats) + 0.75);
  const minLon = Math.max(-179.5, Math.min(...lons) - 0.75);
  const maxLon = Math.min(179.5, Math.max(...lons) + 0.75);
...
async function boot() {
  ...
  const baitBBox = locationsBbox(locations);
  if (baitBBox) {
    const bait = await getJsonSafe(`/gfs/api/bait?bbox=${encodeURIComponent(baitBBox)}&quality=coarse`, null);
    renderBaitZones({ payload: bait, map3DElement: globeEl });
  }

  statusEl.textContent = `Ready • ${locations.length} fish beacons`;
  startLivePolling();
  gfsSocket.connect();
}
