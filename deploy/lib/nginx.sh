...
install_nginx_config() {
  ...
  local gfs_api_upstream="${GFS_UPSTREAM_HOST}:${GFS_UPSTREAM_PORT}"

  local gfs_locations="location = /gfs {\n        return 302 /gfs/;\n    }\n\n    location /gfs/ {\n        alias ${APP_DIR}/;\n        index indexgfs.html;\n        try_files \\\$uri \\\$uri/ /indexgfs.html;\n        add_header Cache-Control \"no-store, no-cache, must-revalidate, proxy-revalidate\";\n    }\n\n    location = /indexgfs.html {\n        root ${APP_DIR};\n        add_header Cache-Control \"no-store, no-cache, must-revalidate, proxy-revalidate\";\n    }\n\n    location = /fishloclist.csv {\n        alias ${APP_DIR}/fishloclist.csv;\n        default_type text/csv;\n        access_log off;\n        add_header Cache-Control \"no-store, no-cache, must-revalidate, proxy-revalidate\";\n    }\n\n    location = /gfs/fishloclist.csv {\n        alias ${APP_DIR}/fishloclist.csv;\n        default_type text/csv;\n        access_log off;\n        add_header Cache-Control \"no-store, no-cache, must-revalidate, proxy-revalidate\";\n    }\n\n    location /gfs/api/ {\n        proxy_pass http://${gfs_api_upstream}/;\n        proxy_http_version 1.1;\n\n        proxy_set_header Host \\\$host;\n        proxy_set_header X-Real-IP \\\$remote_addr;\n        proxy_set_header X-Forwarded-For \\\$proxy_add_x_forwarded_for;\n        proxy_set_header X-Forwarded-Proto \\\$scheme;\n\n        proxy_buffering off;\n        proxy_cache off;\n        proxy_read_timeout 3600s;\n        proxy_send_timeout 3600s;\n    }\n\n    location /gfs/ws/ {\n        proxy_pass http://${gfs_api_upstream}/ws/;\n        proxy_http_version 1.1;\n\n        proxy_set_header Upgrade \\\$http_upgrade;\n        proxy_set_header Connection \\\$connection_upgrade;\n\n        proxy_set_header Host \\\$host;\n        proxy_set_header X-Real-IP \\\$remote_addr;\n        proxy_set_header X-Forwarded-For \\\$proxy_add_x_forwarded_for;\n        proxy_set_header X-Forwarded-Proto \\\$scheme;\n\n        proxy_buffering off;\n        proxy_cache off;\n        proxy_read_timeout 3600s;\n        proxy_send_timeout 3600s;\n    }"
