...
def _static_file(path_name: str) -> Path:
...
def _static_error_response(app: Quart, exc: Exception):
    app.logger.error("[routes] static deployment invalid: expected directory at %s", STATIC_DIR)
...
def register_routes(app: Quart, state: AppState, settings: Settings, rtc: RTCManager) -> None:
    ...
    async def index():
        try:
            return await send_file(str(_static_file("index.html")))
        except (FileNotFoundError, NotADirectoryError) as exc:
            return _static_error_response(app, exc)
    ...
    async def broadcast():
        try:
            return await send_file(str(_static_file("broadcast.html")))
        except (FileNotFoundError, NotADirectoryError) as exc:
            return _static_error_response(app, exc)
    ...
    async def watch():
        try:
            return await send_file(str(_static_file("watch.html")))
        except (FileNotFoundError, NotADirectoryError) as exc:
            return _static_error_response(app, exc)
    ...
    async def favicon():
        try:
            return await send_file(str(_static_file("favicon.ico")))
    ...
    async def status_dashboard():
        try:
            return await send_file(str(_static_file("status_dashboard.html")))
        except (FileNotFoundError, NotADirectoryError) as exc:
            return _static_error_response(app, exc)
    ...
    async def gfs_page():
        try:
            return await send_file(str(_static_file("indexgfs.html")))
        except (FileNotFoundError, NotADirectoryError) as exc:
            return _static_error_response(app, exc)
