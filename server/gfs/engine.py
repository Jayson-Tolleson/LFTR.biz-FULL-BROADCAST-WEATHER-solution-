...
class GfsEngine:
    ...
    async def weather_payload(self, intent: RequestIntent) -> dict[str, Any]:
        ...
        if cached:
            return cached
        raw, valid_time, stale = await self._fetch_atmospheric(WEATHER_VARIABLES, intent)
    ...
    async def clouds_payload(self, intent: RequestIntent) -> dict[str, Any]:
        raw, valid_time, stale = await self._fetch_atmospheric(CLOUD_VARIABLES, intent)
    ...
    async def bait_payload(self, intent: RequestIntent) -> dict[str, Any]:
        atmospheric, valid_time, stale = await self._fetch_atmospheric(BAIT_ATMOSPHERIC_VARIABLES, intent)
    ...
    async def _fetch_atmospheric(self, variables: tuple[str, ...], intent: RequestIntent) -> tuple[dict[str, Any], datetime | None, bool]:
        merged: dict[str, Any] = {}
        ...
        try:
            ...
            stale_payload = self.snapshot.payloads.get("weather") if self.snapshot.payloads else None
            if stale_payload:
                stale = True
                return stale_payload.get("fields", {}), self.snapshot.last_valid_time, True
            raise
        return merged, source_time, stale
