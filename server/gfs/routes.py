...
def create_gfs_blueprint(static_dir: Path) -> Blueprint:
    ...
    async def gfs_location(location_id: str):
        try:
            ...
            log.warning("location not found path=%s location_id=%s", request.path, location_id)
            return jsonify({"ok": False, "error": "location_not_found", "location_id": location_id}), 404
    ...
    async def gfs_location_live_get(location_id: str):
        try:
            ...
            log.warning("live location not found path=%s location_id=%s", request.path, location_id)
            return jsonify({"ok": False, "error": "location_not_found", "location_id": location_id}), 404
        except GfsError as exc:
    ...
    async def gfs_location_live_post(location_id: str):
        try:
            ...
            log.warning("live update location not found path=%s location_id=%s", request.path, location_id)
            return jsonify({"ok": False, "error": "location_not_found", "location_id": location_id}), 404
        except GfsError as exc:
    ...
    async def gfs_bait_advanced():
        try:
            intent = engine().parse_intent(request.args)
            return jsonify(await engine().bait_advanced_payload(intent))
        except GfsError as exc:
            log.error("bait advanced route failed path=%s args=%s err=%s", request.path, dict(request.args), exc, exc_info=True)
            return jsonify(exc.to_json()), exc.status_code
