...
class GFSService:
    ...
    def ingest_latest_model_fields(self, bbox: dict[str, float]) -> dict[str, Any]:
        ...
        with self._ingest_lock:
            ...
            try:
                ...
                if isinstance(lkg.get("groups"), dict) and lkg.get("groups"):
                    lkg_fetch = FetchResult(
                        ok=True,
                        path=Path(lkg.get("fetch", {}).get("path")) if lkg.get("fetch", {}).get("path") else None,
                    ...
                    print("[gfs-ingest] using last-known-good decoded GRIB2 state")
                    self._update_ingest_state_success(lkg_fetch, lkg.get("groups") or {}, mode="last_known_good", error=str(exc))
                    return {"mode": "last_known_good", "fetch": lkg_fetch, "groups": lkg.get("groups") or {}, "bbox": lkg.get("bbox") or bbox, "error": str(exc)}
    ...
    def _scene_cache_key(self, bbox: dict[str, float]) -> str:
        return f"scene:{round(float(bbox.get('west',-180)),3)}:{round(float(bbox.get('south',-80)),3)}:{round(float(bbox.get('east',180)),3)}:{round(float(bbox.get('north',80)),3)}"

    def _get_cached_scene_payload(self, bbox: dict[str, float]) -> tuple[dict[str, Any], dict[str, Any]]:
        now_ms = self._now_ms()
        ttl_ms = max(10_000, int(self.state.cache_ttl_seconds * 1000))
    ...
    def layer_tile_payload(self, layer: str, z: int, x: int, y: int, pad_deg: float = 0.18, debug: bool = False) -> dict[str, Any]:
        ...
        caps = {"clouds": 220, "precip": 240, "lightning": 160, "wind": 220, "swell": 160, "fish": 260}
