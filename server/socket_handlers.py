...
def register_socket_handlers(sio, state: AppState, settings: Settings, rtc: RTCManager) -> None:
    ...
    async def set_room_settings(sid, data):
        ...
        if "ai_mode" in payload:
            mode = str(payload.get("ai_mode") or "active").strip().lower()
            room.settings.ai_mode = "idle" if mode == "idle" else "active"
    ...
    async def chat_message(sid, data):
        ...
        if not room.settings.ai_enabled:
            return
        if room.settings.ai_mode == "idle" and sender != "broadcaster":
            return

        ai_payload = {"text": text, "mode": room.settings.ai_mode, "room": room_id}
        ai_result = await ai.handle_chat(ai_payload, settings.ai_fallback_text)
        reply_raw = (((ai_result or {}).get("data") or {}).get("reply") or settings.ai_fallback_text or "").strip()
        if not reply_raw:
            return
        if room.settings.ai_mode == "idle":
            reply = f"{reply_raw.splitlines()[0][:180]}"
        else:
            reply = f"{reply_raw.splitlines()[0][:260]}"
        await sio.emit(
            "chat_message",
            {
                "sender": "assistant",
    ...
    async def web_search(sid, data):
        ...
        if not query:
            return
        result = await ai.handle_websearch({"query": query})
        await sio.emit(
            "web_search_result",
            {
                "query": query,
                ...
                "results": (((result or {}).get("data") or {}).get("results") or []),
                "ts": now_ms(),
            },
            to=f"room:{room_id}:all",
        )
    ...
    async def webrtc_offer(sid, data):
        ...
        try:
            ...
            log.exception("webrtc_offer failed room=%s sid=%s", room_id, sid)
            await sio.emit("webrtc_error", {"stage": "offer", "message": str(exc), "ts": now_ms()}, to=sid)
    ...
    async def webrtc_ice(sid, data):
        ...
        try:
            ...
            log.exception("webrtc_ice failed room=%s sid=%s", room_id, sid)
    ...
    async def watch_join(sid, data):
        ...
        try:
            ...
            log.exception("watch_join offer failed room=%s sid=%s", room_id, sid)
            await sio.emit("webrtc_error", {"stage": "watch_join", "message": str(exc), "ts": now_ms()}, to=sid)
            return
        room = state.ensure_room(room_id)
        await _emit_stage_state(sio, room_id, room)
    ...
    async def watch_answer(sid, data):
        ...
        sdp_type = (data or {}).get("type")
        if not sdp or not sdp_type:
            log.warning("watch_answer missing SDP room=%s sid=%s", room_id, sid)
            return
        try:
